...
func init() {
    connectionString := "mongodb+srv://your_mongo_db_url/?retryWrites=true&w=majority"
    if connectionString == "" {
...
// Handle postCustomInfo POST request concurrently
func PostCustomInfo(w http.ResponseWriter, r *http.Request) {
    startTime := time.Now()
    ...
    // Unmarshal the JSON body into the CustomInfo struct
    var customInfo CustomInfo
    if err := json.Unmarshal(body, &customInfo); err != nil {
        handleError(w, "Error decoding request body", http.StatusBadRequest)
        return
    }

    // Use a Goroutine to handle the customInfo concurrently
    go func() {
        handleCustomInfoConcurrently(w, customInfo)
    }()
...
func handleCustomInfoConcurrently(w http.ResponseWriter, customInfo CustomInfo) {
    var wg sync.WaitGroup
    wg.Add(1)
    defer wg.Done()

    collection := client.Database("test").Collection("customInfo")

    ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
    defer cancel()

    // Check if the customInfo already exists
    existingCustomInfo := &CustomInfo{}
    err := collection.FindOne(ctx, bson.M{"email": customInfo.Email}).Decode(existingCustomInfo)

    if err == nil {
        updateCustomInfoEntry(w, collection, customInfo)
    } else if err == mongo.ErrNoDocuments {
        createCustomInfoEntry(w, collection, customInfo)
    } else {
        log.Printf("Error fetching customInfo data from MongoDB: %v", err)
    }

    wg.Wait() // Wait for the spawned Goroutine to finish
}

func createCustomInfoEntry(w http.ResponseWriter, collection *mongo.Collection, customInfo CustomInfo) {
    // Insert the new document into MongoDB
    if _, err := collection.InsertOne(context.Background(), customInfo); err != nil {
        handleMongoError(w, customInfo.Email, err)
        return
    }

    log.Printf("Success: New customInfo entry created for %s", customInfo.Email)

    response := map[string]interface{}{"message": "New customInfo entry created"}
    respondJSON(w, http.StatusOK, response)
}

func updateCustomInfoEntry(w http.ResponseWriter, collection *mongo.Collection, customInfo CustomInfo) {
    // Try to update the existing document in MongoDB
    updateResult, err := collection.UpdateOne(
        context.Background(),
        bson.M{"email": customInfo.Email},
        bson.M{"$set": bson.M{"customImage": customInfo.CustomImage, "userName": customInfo.UserName}},
    )
    if err != nil {
        handleMongoError(w, customInfo.Email, err)
        log.Printf("Error updating customInfo entry for %s: %v", customInfo.Email, err)
        return
    }

    if err == nil && updateResult.MatchedCount > 0 {
        log.Printf("Success: Updated customInfo entry for %s", customInfo.Email)
        response := map[string]interface{}{"message": "CustomInfo entry updated"}
        respondJSON(w, http.StatusOK, response)
        return
    }
